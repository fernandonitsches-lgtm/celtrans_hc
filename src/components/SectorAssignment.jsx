...
const SectorAssignment = () => {
  ...
  useEffect(() => {
    if (initialPeople.length > 0 && operacoes.length > 0) {
      initializeAssignments(initialPeople, operacoes);
      const expandAll = {};
  ...
  const handleDrop = (target) => {
    ...
    setAssignments(prev => {
      ...
      if (!newAssignments[target]) newAssignments[target] = [];
  ...
  const handleJustificativa = (personId, texto) => {
    setJustificativas(prev => ({ ...prev, [personId]: texto }));
  };

  const handleSalvar = () => setModalAberto(true);

  const handleConfirmarSalvar = async (justificativasFinais, planoAcao, impedimentos) => {
    setSalvando(true);
    try {
      const atribuicoes = [];
      ...
      if (atribuicoes.length > 0) {
        const { error: atribError } = await supabase.from('atribuicoes_diarias').insert(atribuicoes);
        if (atribError) throw atribError;
      }

      if (faltas.length > 0) {
        const { error: faltaError } = await supabase.from('faltas_diarias').insert(faltas);
  ...
  const pessoasNoSetor = (operacao, setor) => assignments[makeKey(operacao, setor)] || [];
  const pessoasOriginaisDaOperacaoNoSetor = (setor, operacao) => assignments[makeKey(operacao, setor)] || [];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4">
      <div className="max-w-full mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <input type="date" value={today} onChange={(e) => setToday(e.target.value)} className="px-3 py-2 border border-slate-300 rounded-lg text-sm" />
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-3 mb-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-2 flex-wrap">
              <button onClick={handleSalvar} className="flex items-center gap-2 px-3 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 transition">
                <Download className="w-4 h-4" /> Salvar
              </button>
              <button onClick={() => setViewMode('dashboard')} className="flex items-center gap-2 px-3 py-2 bg-purple-600 text-white text-sm rounded-lg hover:bg-purple-700 transition">
                <BarChart3 className="w-4 h-4" /> Dashboard
              </button>
              <button onClick={handleReset} className="flex items-center gap-2 px-3 py-2 bg-slate-500 text-white text-sm rounded-lg hover:bg-slate-600 transition">
                <RotateCcw className="w-4 h-4" /> Restaurar
              </button>
              <button onClick={handleExport} className="flex items-center gap-2 px-3 py-2 bg-teal-600 text-white text-sm rounded-lg hover:bg-teal-700 transition">
        ...
        <div className="bg-white rounded-lg shadow-md p-3 mb-4">
          <div className="flex flex-col md:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input type="text" placeholder="Buscar por nome ou cargo..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              ...
              .map(operacao => (
                <div key={operacao} className="bg-white rounded-lg shadow-md overflow-hidden">
                  <button onClick={() => setExpandedOps(prev => ({ ...prev, [operacao]: !prev[operacao] }))}
                    ...
                    <div className="flex items-center gap-3">
                      <ChevronDown className={`w-5 h-5 transition-transform ${expandedOps[operacao] ? '' : '-rotate-90'}`} />
                      ...
                      <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full">
                        {setoresPorOperacao(operacao).reduce((sum, setor) => {
                          ...
                          const semFalta = daqui.filter(p => !assignments['falta']?.some(f => f.id === p.id));
                    ...
                    <div className="border-t border-slate-200 p-4">
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                        {setoresPorOperacao(operacao).map(setor => {
                          ...
                          const semFalta = todas.filter(p => !assignments['falta']?.some(f => f.id === p.id));
                          ...
                          return (
                            <div key={setor}
                              ...
                              className={`rounded-lg border-2 border-dashed p-3 min-h-64 transition-colors ${
                                dragOver === makeKey(operacao, setor) ? 'bg-blue-100 border-blue-500 shadow-inner' : 'bg-blue-50 border-blue-200'
                              }`}>
                              <h3 className="font-bold text-slate-700 mb-2 pb-2 border-b-2 border-blue-300 text-xs line-clamp-2">{setor}</h3>
                              <h4 className="text-xs font-semibold text-blue-600 mb-2">({filtered.length})</h4>
                              <div className="space-y-2">
                                {filtered.map(person => {
                                  const visitante = estaForaDaOrigem(person, operacao, setor);
                                  return (
                                    <div key={person.id} draggable onDragStart={() => handleDragStart(person, makeKey(operacao, setor))}
                                      ...
                                      className={`p-2 rounded cursor-grab active:cursor-grabbing hover:shadow-md active:opacity-50 active:scale-95 transition-all text-xs ${
                                        visitante ? 'bg-amber-50 border-l-4 border-amber-400 border border-amber-200' : 'bg-white border-l-4 border-blue-500'
                                      }`}>
                                      ...
                                      {visitante && <div className="text-amber-600 text-xs mt-0.5 font-medium">↪ remanejado</div>}
                                    </div>
                                  );
                                })}
                                {ghosts.map(person => (
                                  <div key={`ghost-${person.id}`} title={`${person.name} está em outro setor`}
                                    className="bg-white p-2 rounded border-l-4 border-blue-300 text-xs opacity-50 select-none" style={{ borderStyle: 'dashed' }}>
                                ...
                                {vagas.filter(v => v.setor === setor && v.operacao === operacao).map(vaga => (
                                  <div key={`vaga-${vaga.id}`} className="bg-purple-50 p-2 rounded border-2 border-dashed border-purple-400 text-xs">
            ...
            {filterOperacao === 'todas' && (
              <div className="bg-white rounded-lg shadow-md overflow-hidden">
                <button onClick={() => setExpandedOps(prev => ({ ...prev, [OP_SUPORTE]: !prev[OP_SUPORTE] }))}
                  ...
                  <div className="flex items-center gap-3">
                    <ChevronDown className={`w-5 h-5 transition-transform ${expandedOps[OP_SUPORTE] ? '' : '-rotate-90'}`} />
                    <span className="font-bold text-teal-700">SUPORTE</span>
                    <span className="text-xs bg-teal-100 text-teal-700 px-2 py-1 rounded-full">{pessoasSuporte.length} pessoas</span>
                    <span className="text-xs bg-slate-100 text-slate-500 px-2 py-1 rounded-full italic">não conta no absenteísmo</span>
                  </div>
                </button>
                {expandedOps[OP_SUPORTE] && (
                  <div className="border-t border-slate-200 p-4">
                    {pessoasSuporte.length === 0 ? (
                      <p className="text-sm text-slate-400 italic text-center py-4">Nenhuma pessoa cadastrada com operação "SUPORTE" ainda.</p>
                    ) : (
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                        {setoresSuporte.map(setor => {
                          ...
                          return (
                            <div key={setor} className="bg-teal-50 rounded-lg border-2 border-dashed border-teal-200 p-3 min-h-24">
                              ...
                              <div className="space-y-2">
                                {pessoas.map(person => (
                                  <div key={person.id} className="bg-white border-l-4 border-teal-500 p-2 rounded text-xs select-none">
                                    <div className="font-semibold text-slate-800 line-clamp-2">{person.name}</div>
                                    <div className="text-slate-600 text-xs mt-0.5 line-clamp-1">{person.cargo}</div>
                                  </div>
                                ))}
          ...
          <div className="w-56 flex-shrink-0 flex flex-col gap-4 sticky top-20">
            <div className="bg-red-50 rounded-lg shadow-md border-2 border-dashed border-red-300 overflow-hidden">
              <div className="bg-red-500 px-3 py-2 flex items-center justify-between">
                ...
                <span className="bg-red-700 text-white text-xs font-bold px-2 py-0.5 rounded-full">{filteredPeople(assignments['falta'])?.length || 0}</span>
              </div>
              <div onDragOver={(e) => handleDragOver(e, 'falta')} onDragLeave={handleDragLeave} onDrop={() => handleDrop('falta')}
                className={`p-2 min-h-24 space-y-1 transition-colors rounded-b-lg ${dragOver === 'falta' ? 'bg-red-100' : ''}`}>
                {filteredPeople(assignments['falta'])?.length === 0 && <p className="text-xs text-red-300 italic text-center mt-4">Arraste aqui</p>}
                {filteredPeople(assignments['falta'])?.map(person => (
                  <div key={person.id} draggable onDragStart={() => handleDragStart(person, 'falta')}
            ...
            <div className="bg-orange-50 rounded-lg shadow-md border-2 border-orange-200 overflow-hidden">
              <div className="bg-orange-400 px-3 py-2 flex items-center justify-between">
                ...
                <span className="bg-orange-600 text-white text-xs font-bold px-2 py-0.5 rounded-full">{pessoasEmFerias.length}</span>
              </div>
              <div className="p-2 min-h-16 space-y-1">
                {pessoasEmFerias.length === 0 && <p className="text-xs text-orange-300 italic text-center mt-4">Nenhum em férias</p>}
                {pessoasEmFerias.map(person => (
                  <div key={person.id} className="bg-white border-l-4 border-orange-400 p-2 rounded text-xs select-none">
            ...
            <div className="bg-purple-50 rounded-lg shadow-md border-2 border-purple-200 overflow-hidden">
              <div className="bg-purple-500 px-3 py-2 flex items-center justify-between">
                ...
                <span className="bg-purple-700 text-white text-xs font-bold px-2 py-0.5 rounded-full">{vagas.length}</span>
              </div>
              <div className="p-2 min-h-16 space-y-1">
                {vagas.length === 0 && <p className="text-xs text-purple-300 italic text-center mt-4">Nenhuma vaga aberta</p>}
                {vagas.map(vaga => (
                  <div key={vaga.id} className="bg-white border-l-4 border-purple-400 p-2 rounded text-xs select-none">
                    ...
                    {vaga.nome_anterior && <div className="text-purple-400 text-xs mt-0.5">Ant: {vaga.nome_anterior}</div>}
                  </div>
                ))}
              </div>
            </div>
        ...
        {confirmarReset && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl p-6 max-w-sm w-full mx-4">
              ...
              <p className="text-slate-600 text-sm mb-6">Todas as alterações feitas serão perdidas.</p>
              <div className="flex justify-end gap-3">
                <button onClick={() => setConfirmarReset(false)} className="px-4 py-2 bg-slate-200 text-slate-700 rounded-lg hover:bg-slate-300 transition font-semibold">Cancelar</button>
                <button onClick={handleConfirmarReset} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition font-semibold">Restaurar</button>
              </div>
            </div>
          </div>
        )}
